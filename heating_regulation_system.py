...
class HeatingRegulationSystem:
    ...
    def compute_time_shifts(self):
        ...
        for i in range(len(self.times_occupancy)):
            ...
            if i % 2 == 0: # heating time
                T_end = self.T_heat
                t_star = self.temperature_evolution.compute_warmup_time(self.T_heat, self.T_cool, T_start, time_shifts[-1], self.times_occupancy[i])
            else: # cooling time
                T_end = self.T_cool
                t_star = self.temperature_evolution.compute_cooldown_time(self.T_heat, self.T_cool, T_start, time_shifts[-1], self.times_occupancy[i])

            time_shifts.append(floor(t_star))

            # Simulate for the considered period to get the new starting temperature
            time_span_period, temperature_span_period = self.temperature_evolution.simulate_heat_transfer_over_period(T_start, T_end, time_shifts[-2], time_shifts[-1])
    ...
    def control_heating(self):
        ...
        for i in range(len(self.room.bookings)):
            ...
            # Compute warmump and cooldown times for this period in minutes
